...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<User>> Register(CreateUserRequest request)
    {
        ...
        if (request.Password != request.ConfirmPassword) return BadRequest("As senhas não conferem.");
    ...
    private string CreateToken(User user)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(ClaimTypes.Name, user.UserName), // Usa a propriedade 'UserName'
            new(ClaimTypes.Email, user.Email),
            new(ClaimTypes.Role, user.UserType ?? "User") // Usa a propriedade 'UserType'
        };
